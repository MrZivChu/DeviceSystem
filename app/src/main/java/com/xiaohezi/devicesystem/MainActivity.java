...
public class MainActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    void OnPreview() {
        ...
        if (previewHandle_ < 0) {
            Toast.makeText(MainActivity.this, "NET_DVR_RealPlay_V40 fail, Err:" + CameraHelper.GetLastError(), Toast.LENGTH_SHORT).show();
        } else {
    ...
    public void surfaceCreated(@NonNull SurfaceHolder surfaceHolder) {
        ...
        if (surface.isValid()) {
            if (-1 == CameraHelper.OnRealPlaySurfaceChanged(previewHandle_, 0, surfaceView_))
                Toast.makeText(MainActivity.this, "NET_DVR_PlayBackSurfaceChanged" + CameraHelper.GetLastError(), Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void surfaceDestroyed(@NonNull SurfaceHolder surfaceHolder) {
        ...
        if (surfaceView_.getHolder().getSurface().isValid()) {
            if (-1 == CameraHelper.OnRealPlaySurfaceChanged(previewHandle_, 0, null)) {
                Toast.makeText(MainActivity.this, "NET_DVR_RealPlaySurfaceChanged" + CameraHelper.GetLastError(), Toast.LENGTH_SHORT).show();
            }
        }
    }
}